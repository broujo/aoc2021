...
let rec solve sensors =
  ...
  | t :: [] -> CCList.length t
  | t :: q -> begin
      let rr = CCList.find_map (fun si -> find_inter_rotate t si |> CCOption.map (fun (a,b) -> (si,a,b))) q in
      match rr with
      | None -> failwith "no result :("
      | Some(si, rot, sensor) ->
          let si_s0 = CCList.map (fun v -> add_vect sensor (mult_vect rot v)) si in
          let s0 = CCList.sort_uniq ~cmp:compare (t @ si_s0) in
          let q = CCList.remove ~eq:(=) ~key:si q in
          solve (s0 :: q)
  end
    ...
    | t :: q, _ -> begin
        ...
        let rrl_s0 =
          CCList.map (fun (si, rot, sensor) ->
              let si_s0 = CCList.map (fun v -> add_vect sensor (mult_vect rot v)) si in
              si_s0) rrl in
        let found' = CCList.fold_right (fun sis0 acc -> CCList.sort_uniq ~cmp:compare (sis0 @ acc)) rrl_s0 found in
